...
export default function DashboardPage() {
  ...
  const filteredCases = cases.filter((c) => {
    ...
    if (filterStatus === 'ACTIVE') return c.status !== 'RECOVERED' && c.status !== 'STOPPED' && c.status !== 'HUMAN_REVIEW';
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-800 pb-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white">Merchant Recovery Control Center</h1>
          <p className="text-xs md:text-sm text-slate-400 mt-1">
            Real-time bounded decision engine tracking failure diagnosis, policy verification, and recovered revenue.
          </p>
        </div>
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            className="px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white rounded-xl text-xs font-semibold shadow-lg shadow-blue-600/25 transition flex items-center space-x-1.5"
          >
            {creatingTest ? (
              <span>Ingesting Failure...</span>
            ) : (
              <>
                <span>⚡ Ingest Test Recovery (₹12,000)</span>
              </>
            )}
          </button>
        </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-xl overflow-hidden">
        <div className="p-5 border-b border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-base font-bold text-white tracking-tight">Recovery Opportunities</h2>
        ...
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-400">
            ...
            <p className="text-xs text-slate-400 mb-4">Click "Ingest Test Recovery" above to process a failure event.</p>
            <button
              onClick={() => handleCreateTestRecovery(1200000, 'GATEWAY_TIMEOUT')}
